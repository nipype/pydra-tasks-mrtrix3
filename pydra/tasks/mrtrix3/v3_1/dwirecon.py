...
@shell.define
class DwiRecon(shell.Task["DwiRecon.Outputs"]):
    ...
    operation: str = shell.arg(
        argstr="",
        position=2,
        help="""the way in which output DWIs will be reconstructed; one of: combine_pairs, combine_predicted""",
        allowed_values=["combine_pairs", "combine_predicted"],
    )

    # Options

    # Options common to multiple operations:
    field: ImageIn | None = shell.arg(
        default=None,
        argstr="-field",
        help="""provide a B0 field offset image in Hz""",
    )

    # Options specific to "combine_pairs" operation:
    pairs_in: File | None = shell.arg(
        default=None,
        argstr="-pairs_in",
        help="""provide a text file specifying the volume indices that should be considered paired and therefore combined""",
    )
    ...
    lmax: list[int] | None = shell.arg(
        default=None,
        argstr="-lmax",
        help="""set the maximal spherical harmonic degrees to use (one for each b-value) during signal reconstruction""",
        sep=",",
    )
    exponent: float | None = shell.arg(
        default=None,
        argstr="-exponent",
        help="""set the exponent modulating relative contributions between empirical and predicted signal (see Description)""",
    )
    ...
    class Outputs(shell.Outputs):
        out_file: ImageOut = shell.outarg(
            ...
            help="""the output DWI series""",
        )
        pairs_out: File | bool | None = shell.outarg(
            default=None,
            argstr="-pairs_out",
            path_template="pairs_out.txt",
            help="""output a text file encoding which input volumes were combined in production of the output volumes""",
        )
        weights: ImageOut | bool | None = shell.outarg(
            default=None,
            argstr="-weights",
            path_template="weights.mif",
            help="""export an image encoding, for each intensity sample,how much the output was weighted by the empirical data vs. prediction""",
        )
        predicted: ImageOut | bool | None = shell.outarg(
            default=None,
            argstr="-predicted",
            path_template="predicted.mif",
            help="""export an image containing the predictions generated excluding each volume's own phase encoding group""",
        )
